                ...
                return (
                  <div key={`curriculum-progress-row-${card.courseName}`} className={`dr-curriculum-progress-row dr-curriculum-progress-row-${card.statusKind}`} style={rowStyle}>
                    <div className="dr-curriculum-progress-row-title">
                      <strong className={getCourseColorClass(card.courseName)}>{card.courseName}</strong>
                      <span>
                        {card.gap === null
                          ? 'veri yok'
                          : card.gap === 0
                ...
                {learningRows.map((row) => (
                  <div key={`learning-row-${row.key || row.courseName + row.unitName + row.topicName}`} className="grid gap-3 border-b border-slate-900/5 px-4 py-3 text-xs last:border-b-0 lg:grid-cols-[minmax(160px,1.5fr)_80px_90px_80px_100px_120px] lg:items-center">
                    <div className="min-w-0">
                      <div className="break-words font-bold text-slate-900 dark:text-white">{row.topicName}</div>
                      <div className="mt-0.5 break-words text-[11px] font-semibold text-slate-500"><span className={getCourseColorClass(row.courseName)}>{row.courseName}</span> / {row.unitName}</div>
                    </div>
                    <div className="font-bold text-slate-800 dark:text-slate-100">{row.taskCount || 0}</div>
                    <div className="font-bold text-slate-800 dark:text-slate-100">{Math.floor(Number(row.minutes || 0) / 60)} sa {Number(row.minutes || 0) % 60} dk</div>
