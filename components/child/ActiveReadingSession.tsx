      ...
      {wasTaskDeleted && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-sm">
          <div className="ios-card dr-compact-modal w-[min(22rem,calc(100vw-1.5rem))] p-4 text-center">
            ...
            <h3 className="mb-2 text-xl font-bold">Görev silindi</h3>
      ...
      {showCompleteModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/35 p-3 backdrop-blur-sm">
          <div className="ios-card dr-compact-modal w-[min(26rem,calc(100vw-1.5rem))] p-4">
            <h3 className="text-2xl font-bold text-slate-900">Okuma seansını tamamla</h3>
            <p className="mt-2 text-sm leading-6 text-slate-500">Bu seansta kaç sayfa okuduğunu gir.</p>
            <input type="number" value={pagesRead} onChange={(e) => setPagesRead(e.target.value === '' ? '' : Number(e.target.value))} placeholder="Örn: 25" min="1" autoFocus className="ios-button mt-5 w-full rounded-[24px] px-4 py-4 text-center text-3xl font-bold text-slate-900 focus:outline-none" />
        ...
        <div className="mx-auto flex min-h-full w-full max-w-6xl items-start py-8 md:py-12 px-4">
          <div className="grid w-full gap-6 xl:grid-cols-[minmax(320px,420px)_minmax(0,1fr)]">
            <aside className="dr-session-context-card ios-card rounded-[32px] p-6">
              ...
              <h1 className="mt-2 text-3xl font-bold leading-tight text-slate-900">{task.bookTitle || task.title}</h1>
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                <div className="ios-widget ios-blue rounded-[22px] p-4"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">Okuma</div><div className="mt-2 text-xl font-bold text-slate-900">{formatTime(mainTime)}</div></div>
                <div className="ios-widget ios-yellow rounded-[22px] p-4"><div className="text-xs font-bold uppercase tracking-wide text-amber-600">Mola</div><div className="mt-2 text-xl font-bold text-amber-800">{formatTime(breakTime)}</div></div>
                <div className="ios-widget rounded-[22px] p-4"><div className="text-xs font-bold uppercase tracking-wide text-slate-400">Duraklat</div><div className="mt-2 text-xl font-bold text-slate-700">{formatTime(pauseTime)}</div></div>
              </div>
            ...
            <section className="ios-card rounded-[36px] p-6">
              ...
              <div className="flex min-h-[420px] flex-col items-center justify-center">
                ...
                <div className="mt-8 grid w-full max-w-2xl gap-3 md:grid-cols-3">
                  <div className="ios-widget ios-blue rounded-[22px] p-4 text-center"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">Plan</div><div className="mt-2 text-2xl font-bold text-slate-900">{task.plannedDuration} dk</div></div>
                  <div className="ios-widget ios-yellow rounded-[22px] p-4 text-center"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">Mola</div><div className="mt-2 text-2xl font-bold text-slate-900">{formatTime(breakTime)}</div></div>
                  <div className="ios-widget ios-mint rounded-[22px] p-4 text-center"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">Durum</div><div className="mt-2 text-2xl font-bold text-slate-900">{status === 'running' ? 'Akış' : status === 'break' ? 'Mola' : 'Bekle'}</div></div>
                </div>
              </div>
            </section>
          </div>
