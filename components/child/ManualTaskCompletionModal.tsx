...
const ManualTaskCompletionModal: React.FC<ManualTaskCompletionModalProps> = ({ show, onClose, task, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
      <form className="w-full max-w-xl rounded-[28px] border border-slate-200 bg-white p-6 shadow-2xl" onSubmit={handleSubmit}>
        <div className="mb-5 flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onClose} className="rounded-full bg-slate-100 px-3 py-2 text-sm font-bold text-slate-500 hover:bg-slate-200">Kapat</button>
        </div>
          ...
          <section className="rounded-3xl border border-slate-200 p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="text-sm font-semibold text-slate-700">
                Baslama
                <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-300 px-3 py-3" required />
              </label>
              <label className="text-sm font-semibold text-slate-700">
                Bitis
                <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-300 px-3 py-3" required />
              </label>
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-slate-200 p-4">
            ...
            <div className="space-y-3">
              {task.taskType === 'kitap okuma' && (
                <label className="text-sm font-semibold text-slate-700">
                  Okunan sayfa
                  <input type="number" min="1" value={pagesRead} onChange={(e) => setPagesRead(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-300 px-3 py-3" required />
                </label>
              )}
              ...
              {task.taskType === 'soru çözme' && (
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <label className="text-sm font-semibold text-emerald-700">
                    Dogru
                    <input type="number" min="0" max={task.questionCount} value={correctCount} onChange={(e) => setCorrectCount(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-300 px-3 py-3 text-slate-800" required />
                  </label>
                  <label className="text-sm font-semibold text-rose-700">
                    Yanlis
                    <input type="number" min="0" max={task.questionCount} value={incorrectCount} onChange={(e) => setIncorrectCount(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-300 px-3 py-3 text-slate-800" required />
                  </label>
                  <label className="text-sm font-semibold text-slate-700">
                    Bos
                    <input type="number" min="0" max={task.questionCount} value={emptyCount} onChange={(e) => setEmptyCount(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-300 px-3 py-3 text-slate-800" required />
                  </label>
                  <div className="sm:col-span-3 rounded-2xl bg-slate-50 px-3 py-3 text-sm text-slate-600">Toplam: <strong>{totalQuestions}</strong> / {task.questionCount}</div>
                </div>
              )}

              {task.taskType === 'ders çalışma' && <div className="rounded-2xl bg-slate-50 px-3 py-3 text-sm text-slate-600">Bu gorevde sure bilgisi yeterli. Odak ve verim sonraki analizde hesaplanir.</div>}
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
          <button type="button" onClick={onClose} className="rounded-2xl bg-slate-100 px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-200">Iptal</button>
          <button type="submit" className="rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white hover:bg-emerald-700">Kaydet ve tamamla</button>
        </div>
      </form>
    </div>
  );
